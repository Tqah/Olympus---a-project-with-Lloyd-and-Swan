...
olymp.word_1 <- subset(olymp.know, select = c(YP04_01))   #Subset words ranked 1st place
olymp.word_2 <- subset(olymp.know, select = c(YP04_02))   #Subset words ranked 2st place
olymp.word_3 <- subset(olymp.know, select = c(YP04_03))   #Subset words ranked 3st place
#To come out with a command that can combine olymp.word_1, olymp_word2 and olymp_word3 into
#a single column (maybe with variable olymp.word.all) for export
